...
if __name__ == "__main__":
    ...
    with open("../data/input/old_baxter_images_webcam_plates/action_webcam_with_april.txt", 'r') as file:
        for i, line in enumerate(file):
            temp = line.split(', ')
            state = np.array(list(map(float, temp[0:4])))
            x = reshape(A[:, :, :, i])
            plt.imshow(resize_image(A[:, :, :, i], 8))
            plt.show()
    exit(0)
    ...
    # #
        ...
        for ii in range(A.shape[-1]):
            ...
            plt.imshow(resize_image(A[:, :, :, ii], 8))
            plt.show()
