...
from s2s.baxter_env import BaxterEnv, BaxterEnvVec, BaxterEnvVAE, BaxterEnvSmall
...
def join(estimators):
    ...
    samples = np.hstack([predicate.sample(10) for predicate in estimators])
    return samples, mask


def show_syms(env, vocab, symbols):

    syms = [find(vocab, x) for x in symbols]

    data, mask = join(syms)
    samples = np.full((10, env.observation_space.shape[0]), np.nan)
    samples[:, mask] = data
...
if __name__ == '__main__':
    ...
    save_dir = '../data/output/baxter_data_small'
